...
const App = () => {
  ...
  const handleExecuteQuery = async () => {
    ...
    try {
      ...
      // 执行任务
      const result = await taskApi.executeTask(currentTask.id);
      
      // 获取更新后的任务详情（包含执行结果）
      const updatedTask = await taskApi.getTask(currentTask.id);
      setCurrentTask(updatedTask);
      
      // 解析执行结果
      if (updatedTask.executionResult) {
        const executionResult = typeof updatedTask.executionResult === 'string' 
          ? JSON.parse(updatedTask.executionResult) 
          : updatedTask.executionResult;
      ...
      const errorMessage = error.response?.data || error.message || '查询执行失败';
  ...
  const handleSaveTask = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data || error.message || '保存任务失败';
  ...
  const handleDeleteTask = async () => {
    ...
    try {
      ...
      const errorMessage = error.response?.data || error.message || '删除任务失败';
      message.error(errorMessage);
    } finally {
      setDeleting(false);
    }
